...
object PlayBuild extends Build {
    ...
    lazy val PlayProject = Project(
        ...
        settings = buildSettingsWithMIMA ++ Seq(
            ...
            scalacOptions ++= Seq("-encoding", "UTF-8", "-Xlint","-deprecation", "-unchecked", "-feature"),
            javacOptions ++= Seq("-source","1.6","-target","1.6", "-encoding", "UTF-8"),
            javaOptions in test ++= Seq("-Xmx1024m", "-XX:MaxPermSize=512m"),
            javacOptions in doc := Seq("-source", "1.6"),
            publishArtifact in packageDoc := buildWithDoc,
            publishArtifact in (Compile, packageSrc) := true,
    ...
    object Dependencies {
      ...
      val jdbcDeps = Seq(
        ...
        val runtime = Seq(
            ...
            "org.fluentlenium"                  %    "fluentlenium-festassert"  %   "0.10.2"    %  "test" exclude("org.jboss.netty", "netty"),
        ...
        val testDependencies = Seq(
            ...
            "com.google.code.findbugs"          %    "jsr305"                   %   "1.3.+",
            "org.fluentlenium"                  %    "fluentlenium-festassert"  %   "0.10.2" exclude("org.jboss.netty", "netty")
        )
